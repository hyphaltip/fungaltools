...
$dbh->sqlite_profile( \&profile );
#$dbh->{Profile} = 2;
my %species;

my $qsth = $dbh->prepare("SELECT DISTINCT query_taxon_id FROM SimilarSequences");
my $t0 = [gettimeofday];
$qsth->execute();
my ($qname,$ct) ;
...
$qsth->finish;
if( $debug ) {
    printf LOG "elapsed for per taxon count %s\n",tv_interval($t0);
}
...
my $pergene_sth = $dbh->prepare("SELECT query_id, subject_id, evalue_mant, evalue_exp, percent_identity, percent_match FROM SimilarSequences WHERE query_id = ? and subject_taxon_id = ? ORDER BY evalue_exp, evalue_mant DESC");
...
for my $sp ( keys %species ) {
  ...
  while ($qsth->fetch ) {
    ...
    $qc_sth->execute($qname, $cutoff_id, $cutoff_pmatch,$qname);
      if( $debug ) {
	  printf LOG "%s elapsed seconds for $qname,$cutoff_id,$cutoff_pmatch, $qname\n", tv_interval ( $t2 );
      }

    print LOG " --> ", join(",", $qname, $cutoff_id, $cutoff_pmatch,$qname), "\n";
...
#    last if $debug && $i++ > 100;
    if ( $debug ) {
	printf LOG "%s Elapsed seconds at bottom of loop\n",tv_interval($t2);
    }
  }
  $qsth->finish;
  if( $debug ) {
      printf LOG "%s elapsed seconds\n", tv_interval ( $t0 );
  }
# generate BRH stats and percent ID stats
  ...
  for my $gn ( keys %{$gene{$sp}} ) {
    ...
    for my $subjtax ( keys %$hits ) {
      ...
      my $t2 = [gettimeofday];
      $pergene_sth->execute( $gn, $subjtax); # reverse subject/query
      if( $debug ) {
	  printf LOG "%s elapsed seconds for execute query for $gn $subjtax\n", tv_interval ( $t2 );
      }

      my ($f_qid, $f_subjid, $f_mant, $f_exp, $f_pid, $f_pmatch);
      my ($p_qid, $p_subjid, $p_mant, $p_exp, $p_pid, $p_pmatch);
      $pergene_sth->bind_columns(\$p_qid, \$p_subjid, 
				 \$p_mant, \$p_exp, \$p_pid, \$p_pmatch);      
      while ( $pergene_sth->fetch ) {
	if ( $f_qid ) {
	  if ( $p_exp < $f_exp) {
	    ($f_qid, $f_subjid, $f_mant, $f_exp, $f_pid, $f_pmatch) = 
		($p_qid, $p_subjid, $p_mant, $p_exp, $p_pid, $p_pmatch);
	  } else {
	    warn("skipping worse hit for\n\t",join("\t",($p_qid, $p_subjid, $p_mant, $p_exp, $p_pid, $p_pmatch)), 
		 " as compared to the better value: \n\t",
		 join("\t",($f_qid, $f_subjid, $f_mant, 
			    $f_exp, $f_pid, $f_pmatch)),"\n") if $debug;
	  }
	} else {
	    ($f_qid, $f_subjid, $f_mant, $f_exp, $f_pid, $f_pmatch) = 
		($p_qid, $p_subjid, $p_mant, $p_exp, $p_pid, $p_pmatch);
      ...
      $pergene_sth->finish;
      $pergene_sth->execute( $sid,$sp); # reverse subject/query
	...
	if ( $r_qid ) {
	  if ( $p_exp < $r_exp) {
	      warn("replacing $r_subjid with ",$p_subjid,"\n");
	    ($r_qid, $r_subjid, $r_mant, $r_exp, $r_pid, $r_pmatch) = 
		($p_qid, $p_subjid, $p_mant, $p_exp, $p_pid, $p_pmatch);
	  } else {
	    warn("skipping worse hit for\n\t",
		 join("\t",($p_qid, $p_subjid, $p_mant, $p_exp, 
			    $p_pid, $p_pmatch)),
		 " as compared to the better value: \n\t",
		 join("\t",($r_qid, $r_subjid, $r_mant, $r_exp, 
			    $r_pid, $r_pmatch)),"\n") if $debug;
	  }
	} else {
	  ($r_qid, $r_subjid, $r_mant, $r_exp, 
	   $r_pid, $r_pmatch) = ($p_qid, $p_subjid, $p_mant, 
				 $p_exp, $p_pid, $p_pmatch);
	}
      }
      $pergene_sth->finish;
      unless ( $r_qid ) {
	# skip this one the unidirectional search is 
	# hitting a false positive it seems
	  warn("$f_qid $f_subjid does not have a reverse hit for $f_subjid\n");
	next;
      }

      my $brh = $f_qid eq $r_subjid ? 'YES' : 'NO';
      print $ofh join("\t", $f_qid, $f_subjid, sprintf("%.1fe%d",$f_mant,$f_exp),
		      sprintf("%.1f",$f_pid), sprintf("%.1f",$f_pmatch),
		      $brh,
		      $r_qid, $r_subjid, sprintf("%.1fe%d",$r_mant,$r_exp),
		      sprintf("%.1f",$r_pid), sprintf("%.1f",$r_pmatch)), "\n";
...
sub profile {
    my ($statement, $time) = @_;
    print LOG "$statement ran for $time seconds\n";
}
END { 
